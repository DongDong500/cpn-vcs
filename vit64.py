...
def add_writer_scalar(writer, phase, score, loss, epoch, num_classes):
    ...
    writer.add_scalar(f'vit64/epoch loss/cls {num_classes}/{phase}', loss, epoch)
    writer.add_scalar(f'vit64/epoch Overall Acc/cls {num_classes}/{phase}', score['Overall Acc'], epoch)
    
    for i in range(num_classes):
        writer.add_scalar(f'vit64/f1 score/cls {num_classes}/{i}/{phase}', score['Class F1'][i], epoch)
...
if __name__ == "__main__":
    ...
    cls = [(256, 256), (128, 128), (64, 64), (32, 32)]
    vit_ps = [256, 128, 64, 32]
    run_time = []

    for i in range( len(cls) ):
        start_time = datetime.now()
        vit(image_patch_size=cls[i], vit_patch_size=vit_ps[i])
