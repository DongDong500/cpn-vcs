...
def add_writer_scalar(writer, phase, score, loss, epoch, num_classes):
    ...
    writer.add_scalar(f'vit16/epoch loss/cls {num_classes}/{phase}', loss, epoch)
    writer.add_scalar(f'vit16/epoch Overall Acc/cls {num_classes}/{phase}', score['Overall Acc'], epoch)
    
    for i in range(num_classes):
        writer.add_scalar(f'vit16/f1 score/cls {num_classes}/{i}/{phase}', score['Class F1'][i], epoch)
...
def vit(image_patch_size:tuple, vit_patch_size:int):
    ...
    dst = CPNvit(root='/DATA/dongik/datasets', datatype='CPN_vit', image_set='train',
                transform=transform, is_rgb=True, dver='splits/v5/3', image_patch_size=patch_size)
    loader = DataLoader(dst, batch_size=8,
                            shuffle=True, num_workers=2, drop_last=True)

    val_dst = CPNvit(root='/DATA/dongik/datasets', datatype='CPN_vit', image_set='test',
                transform=transform, is_rgb=True, dver='splits/v5/3', image_patch_size=patch_size)
...
if __name__ == "__main__":
    ...
    for i in cls:
        ...
        vit(image_patch_size=i, vit_patch_size=16)
