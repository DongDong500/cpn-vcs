...
class Medianpadw(data.Dataset):
    ...
    def _read(self, index):
        ...
        assert((target.size ==  (640, 640)) or (target.size == (896, 640)))
    ...
    def __init__(self, root, datatype='Median', dver='splits', 
        ...
        image_dir = os.path.join(root, 'Median_padw', 'Images')
        mask_dir = os.path.join(root, 'Median_padw', 'Masks')

        if not os.path.exists(image_dir) or not os.path.exists(mask_dir):
            raise Exception('Dataset not found or corrupted.')
        
        split_f = os.path.join(root, 'Median_padw', dver, image_set.rstrip('\n') + '.txt')
...
if __name__ == "__main__":
    ...
    dst = Medianpadw(root='/data1/sdi/datasets', datatype='Median', image_set='test',
