...
class CPNpad(data.Dataset):
    ...
    def __init__(self, root, datatype='CPN', dver='splits', 
        ...
        image_dir = os.path.join(root, 'CPN_pad', 'Images')
        mask_dir = os.path.join(root, 'CPN_pad', 'Masks')

        if not os.path.exists(image_dir) or not os.path.exists(mask_dir):
            raise Exception('Dataset not found or corrupted.')
        
        split_f = os.path.join(root, 'CPN_pad', dver, image_set.rstrip('\n') + '.txt')
...
if __name__ == "__main__":
    ...
    dst = CPNpad(root='/data1/sdi/datasets', datatype='CPN', image_set='test',
